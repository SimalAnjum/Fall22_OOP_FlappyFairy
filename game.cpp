...
bool Game::init()
{
	...
	else
	{
		...
		//Create window
		gWindow = SDL_CreateWindow( "Flappy Fairy", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
		if( gWindow == NULL )
		{
...
void Game::run()
{
    ...
    while (!quit)
    {
        while (SDL_PollEvent(&e) != 0)
        {
            ...
            {
				loop.fairy_few->handleEvent(e);
				intersected=loop.getstate();
            }
        }
